#############################################################################
# Generated by PAGE version 7.3
#  in conjunction with Tcl version 8.6
#  May 03, 2022 02:05:34 PM PDT  platform: Linux
set vTcl(timestamp) ""
if {![info exists vTcl(borrow)]} {
    ::vTcl::MessageBox -title Error -message  "You must open project files from within PAGE."
    exit}


set image_list { 
}
vTcl:create_project_images $image_list   ;# In image.tcl

if {!$vTcl(borrow) && !$vTcl(template)} {

set vTcl(actual_gui_font_dft_desc)  TkDefaultFont
set vTcl(actual_gui_font_dft_name)  TkDefaultFont
set vTcl(actual_gui_font_text_desc)  TkTextFont
set vTcl(actual_gui_font_text_name)  TkTextFont
set vTcl(actual_gui_font_fixed_desc)  TkFixedFont
set vTcl(actual_gui_font_fixed_name)  TkFixedFont
set vTcl(actual_gui_font_menu_desc)  TkMenuFont
set vTcl(actual_gui_font_menu_name)  TkMenuFont
set vTcl(actual_gui_font_tooltip_desc)  TkDefaultFont
set vTcl(actual_gui_font_tooltip_name)  TkDefaultFont
set vTcl(actual_gui_font_treeview_desc)  TkDefaultFont
set vTcl(actual_gui_font_treeview_name)  TkDefaultFont
set vTcl(actual_gui_bg) #d9d9d9
set vTcl(actual_gui_fg) #000000
set vTcl(actual_gui_analog) #ececec
set vTcl(actual_gui_menu_analog) #ececec
set vTcl(actual_gui_menu_bg) #d9d9d9
set vTcl(actual_gui_menu_fg) #000000
set vTcl(complement_color) #d9d9d9
set vTcl(analog_color_p) #d9d9d9
set vTcl(analog_color_m) #ececec
set vTcl(active_fg) #000000
set vTcl(actual_gui_menu_active_bg)  #ececec
set vTcl(actual_gui_menu_active_fg)  #000000
set vTcl(pr,autoalias) 1
set vTcl(pr,relative_placement) 1
set vTcl(mode) Absolute
}




proc vTclWindow.top2 {base} {
    global vTcl
    if {$base == ""} {
        set base .top2
    }
    if {[winfo exists $base]} {
        wm deiconify $base; return
    }
    set top $base
    set target $base
    ###################
    # CREATING WIDGETS
    ###################
    vTcl::widgets::core::toplevel::createCmd $top -class Toplevel \
        -menu "$top.m51" -background $vTcl(actual_gui_bg) \
        -highlightcolor black 
    wm focusmodel $top passive
    wm geometry $top 1165x696+384+271
    update
    # set in toplevel.wgt.
    global vTcl
    global img_list
    set vTcl(save,dflt,origin) 0
    wm maxsize $top 3825 2130
    wm minsize $top 1 1
    wm overrideredirect $top 0
    wm resizable $top 1 1
    wm deiconify $top
    set toptitle "Vehicle Repair"
    wm title $top $toptitle
    namespace eval ::widgets::${top}::ClassOption {}
    set ::widgets::${top}::ClassOption(-toptitle) $toptitle
    vTcl:DefineAlias "$top" "VehicleSearchForm" vTcl:Toplevel:WidgetProc "" 1
    set vTcl(real_top) {}
    vTcl:withBusyCursor {
    vTcl::widgets::ttk::custom::CreateCmd $top.cus46 \
        -background $vTcl(actual_gui_bg) -height 178 -highlightcolor black \
        -width 1136 
    vTcl:DefineAlias "$top.cus46" "Custom1" vTcl:WidgetProc "VehicleSearchForm" 1
    menu $top.m51 \
        -activebackground $vTcl(actual_gui_menu_analog) \
        -activeforeground #000000 -background $vTcl(actual_gui_menu_bg) \
        -font TkMenuFont -foreground $vTcl(actual_gui_menu_fg) -tearoff 0 
    ttk::style configure TLabelframe.Label -background $vTcl(actual_gui_bg)
    ttk::style configure TLabelframe.Label -foreground $vTcl(actual_gui_fg)
    ttk::style configure TLabelframe.Label -font "$vTcl(actual_gui_font_dft_desc)"
    ttk::style configure TLabelframe -background $vTcl(actual_gui_bg)
    ttk::labelframe $top.tLa47 \
        -text {Enter Vehicle Search Criteria} -relief solid -width 1140 \
        -height 165 
    vTcl:DefineAlias "$top.tLa47" "TLabelframe1" vTcl:WidgetProc "VehicleSearchForm" 1
    set site_3_0 $top.tLa47
    ttk::style configure TButton -background $vTcl(actual_gui_bg)
    ttk::style configure TButton -foreground $vTcl(actual_gui_fg)
    ttk::style configure TButton -font "$vTcl(actual_gui_font_dft_desc)"
    ttk::button $site_3_0.tBu48 \
        -command do_vehicle_search -takefocus {} -text Search -compound left 
    vTcl:DefineAlias "$site_3_0.tBu48" "TButton1" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::entry $site_3_0.tEn49 \
        -font TkTextFont -textvariable yearValue -foreground {} \
        -background {} -takefocus {} -cursor xterm 
    vTcl:DefineAlias "$site_3_0.tEn49" "Year" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::entry $site_3_0.tEn50 \
        -font TkTextFont -textvariable makeValue -foreground {} \
        -background {} -takefocus {} -cursor xterm 
    vTcl:DefineAlias "$site_3_0.tEn50" "Make" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::entry $site_3_0.tEn51 \
        -font TkTextFont -textvariable colorValue -foreground {} \
        -background {} -takefocus {} -cursor fleur 
    vTcl:DefineAlias "$site_3_0.tEn51" "Color" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::label $site_3_0.tLa52 \
        -background $vTcl(actual_gui_bg) -foreground $vTcl(actual_gui_fg) \
        -font TkDefaultFont -relief flat -anchor w -justify left -text Year \
        -compound left 
    vTcl:DefineAlias "$site_3_0.tLa52" "TLabel1" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::label $site_3_0.tLa53 \
        -background $vTcl(actual_gui_bg) -foreground $vTcl(actual_gui_fg) \
        -font TkDefaultFont -relief flat -anchor w -justify left -text Make \
        -compound left 
    vTcl:DefineAlias "$site_3_0.tLa53" "TLabel1_1" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::label $site_3_0.tLa54 \
        -background $vTcl(actual_gui_bg) -foreground $vTcl(actual_gui_fg) \
        -font TkDefaultFont -relief flat -anchor w -justify left -text Model \
        -compound left 
    vTcl:DefineAlias "$site_3_0.tLa54" "TLabel1_1_1" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::entry $site_3_0.tEn62 \
        -font TkTextFont -textvariable vinValue -foreground {} -background {} \
        -takefocus {} -cursor fleur 
    vTcl:DefineAlias "$site_3_0.tEn62" "VinEntry" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::label $site_3_0.tLa63 \
        -background $vTcl(actual_gui_bg) -foreground $vTcl(actual_gui_fg) \
        -font TkDefaultFont -relief flat -anchor w -justify left -text VIN \
        -compound left 
    vTcl:DefineAlias "$site_3_0.tLa63" "TLabel1_1_1_1" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::style configure TButton -background $vTcl(actual_gui_bg)
    ttk::style configure TButton -foreground $vTcl(actual_gui_fg)
    ttk::style configure TButton -font "$vTcl(actual_gui_font_dft_desc)"
    ttk::button $site_3_0.tBu64 \
        -takefocus {} -text Clear -compound left 
    vTcl:DefineAlias "$site_3_0.tBu64" "TButton3" vTcl:WidgetProc "VehicleSearchForm" 1
    place $site_3_0.tBu48 \
        -in $site_3_0 -x 1030 -y 120 -anchor nw -bordermode ignore 
    place $site_3_0.tEn49 \
        -in $site_3_0 -x 150 -y 30 -anchor nw -bordermode ignore 
    place $site_3_0.tEn50 \
        -in $site_3_0 -x 150 -y 60 -anchor nw -bordermode ignore 
    place $site_3_0.tEn51 \
        -in $site_3_0 -x 150 -y 90 -anchor nw -bordermode ignore 
    place $site_3_0.tLa52 \
        -in $site_3_0 -x 20 -y 30 -anchor nw -bordermode ignore 
    place $site_3_0.tLa53 \
        -in $site_3_0 -x 20 -y 60 -width 46 -height 26 -anchor nw \
        -bordermode ignore 
    place $site_3_0.tLa54 \
        -in $site_3_0 -x 20 -y 90 -width 46 -height 26 -anchor nw \
        -bordermode ignore 
    place $site_3_0.tEn62 \
        -in $site_3_0 -x 150 -y 120 -width 164 -height 21 -anchor nw \
        -bordermode ignore 
    place $site_3_0.tLa63 \
        -in $site_3_0 -x 20 -y 120 -width 46 -relwidth 0 -height 26 \
        -relheight 0 -anchor nw -bordermode ignore 
    place $site_3_0.tBu64 \
        -in $site_3_0 -x 940 -y 120 -anchor nw -bordermode ignore 
    vTcl::widgets::ttk::custom::CreateCmd $top.cus49 \
        -background $vTcl(actual_gui_bg) -height 207 -highlightcolor black \
        -width 1148 
    vTcl:DefineAlias "$top.cus49" "Custom2" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::label $top.tLa50 \
        -background $vTcl(actual_gui_bg) -foreground $vTcl(actual_gui_fg) \
        -font {-family {DejaVu Sans} -size 10 -weight bold -slant roman -underline 0 -overstrike 0} \
        -relief flat -anchor w -justify left -text {Vehicle Problems} \
        -textvariable ::vehicleProblemsTitleValue -compound left 
    vTcl:DefineAlias "$top.tLa50" "VehicleProblemsTitle" vTcl:WidgetProc "VehicleSearchForm" 1
    set ::vehicleProblemsTitleValue {Vehicle Problems}
    ttk::label $top.tLa51 \
        -background $vTcl(actual_gui_bg) -foreground $vTcl(actual_gui_fg) \
        -font {-family {DejaVu Sans} -size 10 -weight bold -slant roman -underline 0 -overstrike 0} \
        -relief flat -anchor w -justify left \
        -text {Vehicle Problem Information} -compound left 
    vTcl:DefineAlias "$top.tLa51" "TLabel2_1" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::style configure TButton -background $vTcl(actual_gui_bg)
    ttk::style configure TButton -foreground $vTcl(actual_gui_fg)
    ttk::style configure TButton -font "$vTcl(actual_gui_font_dft_desc)"
    ttk::button $top.tBu52 \
        -command repair_vehicle -takefocus {} -text Repair -compound left 
    vTcl:DefineAlias "$top.tBu52" "RepairButton" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::label $top.tLa53 \
        -background $vTcl(actual_gui_bg) -foreground $vTcl(actual_gui_fg) \
        -font TkDefaultFont -relief flat -anchor w -justify left -text Repair \
        -textvariable ::problemValue -compound left 
    vTcl:DefineAlias "$top.tLa53" "ProblemLabel" vTcl:WidgetProc "VehicleSearchForm" 1
    set ::problemValue {Repair}
    ttk::entry $top.tEn54 \
        -font TkTextFont -textvariable actualCostValue -foreground {} \
        -background {} -takefocus {} -cursor xterm 
    vTcl:DefineAlias "$top.tEn54" "ActaulCostEntry" vTcl:WidgetProc "VehicleSearchForm" 1
    ttk::label $top.tLa55 \
        -background $vTcl(actual_gui_bg) -foreground $vTcl(actual_gui_fg) \
        -font TkDefaultFont -relief flat -anchor w -justify left \
        -text {Actual Cost:} -compound left 
    vTcl:DefineAlias "$top.tLa55" "TLabel3" vTcl:WidgetProc "VehicleSearchForm" 1
    ###################
    # SETTING GEOMETRY
    ###################
    place $top.cus46 \
        -in $top -x 10 -y 172 -width 1136 -relwidth 0 -height 171 \
        -relheight 0 -anchor nw -bordermode ignore 
    place $top.tLa47 \
        -in $top -x 10 -y 10 -width 1141 -relwidth 0 -height 158 -relheight 0 \
        -anchor nw -bordermode ignore 
    place $top.cus49 \
        -in $top -x 10 -y 380 -width 1148 -relwidth 0 -height 207 \
        -relheight 0 -anchor nw -bordermode ignore 
    place $top.tLa50 \
        -in $top -x 10 -y 350 -width 554 -relwidth 0 -height 18 -relheight 0 \
        -anchor nw -bordermode ignore 
    place $top.tLa51 \
        -in $top -x 10 -y 598 -width 322 -relwidth 0 -height 24 -relheight 0 \
        -anchor nw -bordermode ignore 
    place $top.tBu52 \
        -in $top -x 370 -y 657 -width 83 -relwidth 0 -height 28 -relheight 0 \
        -anchor nw -bordermode ignore 
    place $top.tLa53 \
        -in $top -x 10 -y 627 -width 507 -relwidth 0 -height 24 -relheight 0 \
        -anchor nw -bordermode ignore 
    place $top.tEn54 \
        -in $top -x 140 -y 657 -width 164 -relwidth 0 -height 21 -relheight 0 \
        -anchor nw -bordermode ignore 
    place $top.tLa55 \
        -in $top -x 10 -y 657 -width 127 -relwidth 0 -height 24 -relheight 0 \
        -anchor nw -bordermode ignore 
    } ;# end vTcl:withBusyCursor 

    vTcl:FireEvent $base <<Ready>>
}

proc 36 {args} {return 1}


Window show .
set btop1 ""
if {$vTcl(borrow)} {
    set btop1 .bor[expr int([expr rand() * 100])]
    while {[lsearch $btop1 $vTcl(tops)] != -1} {
        set btop1 .bor[expr int([expr rand() * 100])]
    }
}
set vTcl(btop) $btop1
Window show .top2 $btop1
if {$vTcl(borrow)} {
    $btop1 configure -background plum
}

